...
for ct_sel in list(Selectors.CancerTypeSelector):
    ...
    out_path_expr = f'{str(ct_sel)}_expr_specs.txt'
    with open(out_path_expr, 'w') as k:
        ...
        k.write('Align expression data and phenotype data on samples for cohort ' + str(ct_sel) + '...\n')
        ...
        for conf_sel in list(Selectors.ConfounderSelector)
            ...
            i = 0
            for block in conf_partition:
                k.write(f'Block nb {i}: ' + str(len(block))+'\n')
                i += 1
