...
class TestRunner(object):
    ...
    def __init__(self, n, k):
        ...
        self.rnd_partitions = {ct_sel: {conf_sel: Selectors.get_n_random_partitions(self.n, self.pheno_datasets[ct_sel]['submitter_id.samples'], self.conf_partitions[ct_sel][conf_sel], ct_sel, conf_sel)
        ...
        self.rnd_results = {ct_sel: {conf_sel: {alg_sel: {i: list([]) for i in range(0, self.n)} for alg_sel in self.algorithm_selectors} for conf_sel in self.confounder_selectors} for ct_sel in self.cancer_type_selectors}
        self.outfile = ''
        ...
        for alg_sel in alg_selectors:
            ...
            for k in range(10, self.k, 10):
                try:
                    ...
                    print('no more edges')
                    break
            pd.DataFrame({'k': index, 'mean JI': self.conf_results[ct_sel][conf_sel][alg_sel]}).to_csv(os.path.join('results', 'JI', f'cb_{str(alg_sel)}_{str(conf_sel)}_{str(ct_sel)}_jaccInd.csv'), index=False)
